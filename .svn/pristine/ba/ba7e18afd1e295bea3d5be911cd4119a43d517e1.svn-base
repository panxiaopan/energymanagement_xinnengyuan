<template>
  <div class="hj-creat-user-form">
    <el-form ref="userForm" label-position="right" label-width="80px" :model="staffBasePropertyForm" :rules="rules">
    <el-form-item :label="$t('systemSetting.telephone')" prop="tel">
      <el-input v-model="staffBasePropertyForm.tel"  size="small"></el-input>
    </el-form-item>
      <el-form-item :label="$t('systemSetting.loginName')" prop="loginName">
        <el-input v-model="staffBasePropertyForm.loginName" size="small"></el-input>
      </el-form-item>
      <el-form-item :label="$t('systemSetting.nickname')" prop="username">
        <el-input v-model="staffBasePropertyForm.username" size="small"></el-input>
      </el-form-item>

      <el-form-item :label="$t('systemSetting.role')" prop="roleIds">
        <!-- <div > -->
        <el-transfer
        ...
        @change="handleChangePath"></el-transfer>
        <!-- </div> -->
      </el-form-item>
      <el-form-item :label="$t('systemSetting.userGroup')" prop="groupId">
        <div style="display:flex;justify-content:space-between;z-index:1111111;">
          <el-cascader ref="cascader" size="small" :options="options" :props="defaultProps" change-on-select clearable @change="change" v-model="staffBasePropertyForm.groupId" style="width:100%;"></el-cascader>
        </div>
      </el-form-item>
    </el-form>
  </div>
...
export default {
  ...
  data() {
    ...
    var validateLoginName = (rule, value, callback) => {
      if (value === "") {
        callback(new Error(this.$t("systemSetting.inputLoginName")));
      } else {
        var errorString = "";
        switch (true) {
          case !isNaN(+value):
            errorString = this.$t("systemSetting.noAllNumberAtLoginName");
            break;
          case /\s+/.test(value):
            errorString = this.$t("systemSetting.noSpaceAtLoginName");
            break;
          // case  /\W+/g.test(value):
          case /[^A-Za-z0-9_\-\u4e00-\u9fa5]+/g.test(value):
            // errorString = '支持中文,字母,数字, "_", "-"';
            errorString = this.$t("systemSetting.inputCharFormatTips");
            break;
        }
        if (errorString) {
    ...
    var validateUsername = (rule, value, callback) => {
      ...
      if (value == "") {
        callback(new Error(this.$t("systemSetting.inputNickname")));
      } else {
        callback();
      }
    };
    var validateRoleIds = (rule, value, callback) => {
      ...
      if (!value.length) {
        callback(new Error(this.$t("systemSetting.assignRole")));
      } else {
        callback();
      }
    };
    ...
    var telValidator = (rule, value, callback) => {
      ...
      if (value === "" || (value.trim && value.trim().length === 0)) {
        callback(new Error(this.$t("systemSetting.inputPhoneNum")));
      } else if (!(fixedReg.test(value) || mobileReg.test(value))) {
        //手机和固定电话校验
        callback(new Error(this.$t("systemSetting.errorPhoneNum")));
      } else {
        callback();
      }
    };
    return {
      ...
      rules: {
        ...
        tel: [
          {
            require: true,
            message: this.$t("systemSetting.inputPhoneNum"),
            trigger: "change"
          },
          {
            // pattern: /^1[34578]\d{9}$/,
        ...
        roleIds: [
          {
            type: "array",
            require: true,
            message: this.$t("systemSetting.assignRole")
          },
          {
  ...
  methods: {
    ...
    newWindowDialog() {
      ...
      this.$msgbox({
        title: this.$t("systemSetting.addUser"),
        message: h(
          "test-str",
          {
            style: "color: teal",
