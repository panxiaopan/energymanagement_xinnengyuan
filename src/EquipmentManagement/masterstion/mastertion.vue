...
import { provincecity ,townCodestree,addstations,getstationSubTypes,getstationconfig,editstation,deletesubNodes} from "@/api/api";
...
export default {
  ...
  methods: {
                      ...
                      if(res.data.data.subNodes.length>0){
                           for(var i=0;i<res.data.data.subNodes.length;i++){
                            ...
                            this.chirdenstationdata[0].children.push({
                                       id:res.data.data.subNodes[i].id,
   ...
   Delect_sure(){
         ...
         deletesubNodes(this.$route.params.id, this.currentid,parms).then(res=>{
                console.log('删除')
                console.log(res)
                if(res.date.head.code==0){
                      this.$message({
                            type:'success',
                            message:'删除成功'
                      }) 

                    this.$route.push(`/mastertion/add/${this.$route.params.id}/master`)  
                } 
         })   
   },
  },
